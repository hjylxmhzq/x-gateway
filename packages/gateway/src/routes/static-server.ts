...
import {
  ...
  AddStaticServerRequest,
  AddStaticServerRequestValidator
...
import { addStaticServer } from '../services/static-server';

const router = new Router({ prefix: '/static-server' });

router.post('/add-server', async (ctx, next) => {
  const body = ctx.request.body as AddStaticServerRequest;
  try {
    await AddStaticServerRequestValidator.validateAsync(body);
    const proxy = await addStaticServer(body);
    if (proxy) {
...
router.post('/add-static-server', async (ctx, next) => {
